...
function Analyze({ analysis, setAnalysis }) {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(
      `${protocol}//${window.location.host}/api/ws/full-analysis`,
    );
    analysisWs.current = socket;

    const cleanup = () => {
      if (analysisWs.current) {
        analysisWs.current.close();
        analysisWs.current = null;
      }
  ...
  useEffect(() => {
    currentIdxRef.current = currentMoveIndex;
  }, [currentMoveIndex]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
    const socket = new WebSocket(
      `${protocol}//${window.location.host}/api/ws/evaluate`,
    );
    ...
    socket.onmessage = (e) => {
      ...
      if (data.status === "update") setLiveEval(data);
        ...
        console.error("WS eval error:", data.message);
  ...
  const runEvaluation = useCallback(
    (targetFen) => {
      ...
      ws.current.send(
        JSON.stringify({
          fen: targetFen,
          time_limit: depthToTimeMap[evalDepth],
        }),
      );
    },
    [evalDepth],
  ...
  const evalBarPct = useMemo(() => {
    ...
    if (currentMoveIndex >= 0)
      return evalToPercent((gameData[currentMoveIndex]?.evaluation ?? 0) / 100);
    return 50;
      ...
      try {
        ...
        if (move)
          arrows.push([
            move.from,
            move.to,
            i === 0 ? "rgba(201,168,76,0.85)" : "rgba(255,255,255,0.25)",
          ]);
      } catch {}
    });
    return arrows;
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      if (e.key === "ArrowRight") {
        e.preventDefault();
        setCurrentMoveIndex((prev) => Math.min(gameData.length - 1, prev + 1));
        setGuessSubmitted(false);
        setGuessSan("");
      }
      if (e.key === "Home") {
      ...
      if (e.key === "End") {
        e.preventDefault();
        setCurrentMoveIndex(gameData.length - 1);
        setGuessSubmitted(false);
        setGuessSan("");
      }
    };
  ...
  const goToPrevImprovement = useCallback(() => {
    const prev = [...improvementIndices]
      .reverse()
      .find((i) => i < currentMoveIndex);
    if (prev !== undefined) {
  ...
  if (analysisProgress) {
    const moveNum =
      analysisProgress.ply > 0
        ? `${Math.ceil(analysisProgress.ply / 2)}${analysisProgress.ply % 2 !== 0 ? "." : "…"}${analysisProgress.current_move}`
        : "—";
    return (
      <div
        style={{
      ...
      >
        <div
          className="card analysis-progress-modal"
          style={{
            textAlign: "center",
            width: "100%",
            maxWidth: 480,
        ...
        >
          ...
          >
            ...
            <span className="analysis-depth-badge">
              Stockfish {initDepth || "Standard"}
            </span>
          </div>
          ...
          <div className="analysis-progress-body">
            <div className="analysis-stats-row">
              <div className="analysis-stat">
                ...
                <span className="analysis-stat-value mono">
                  {analysisProgress.ply > 0
                    ? `${analysisProgress.ply} / ${analysisProgress.total}`
                    : "—"}
                </span>
              </div>
              <div className="analysis-stat">
          ...
          <div
            className="analysis-progress-footer"
            style={{ justifyContent: "center" }}
          >
            <button className="btn btn-secondary" onClick={cancelAnalysis}>
              Cancel
            </button>
          </div>
        </div>
  ...
  const boardFen = explorationMode
    ? displayFen
    ...
    if (selectedSquare && isDraggable) {
      try {
        ...
        chess.moves({ square: selectedSquare, verbose: true }).forEach((m) => {
          styles[m.to] = {
            background:
              "radial-gradient(circle, transparent 55%, rgba(34,197,94,0.55) 55%, rgba(34,197,94,0.55) 68%, transparent 68%)",
            borderRadius: "50%",
          };
        });
      } catch {}
    ...
    <div className="analyze-layout">
      ...
      <div className="analyze-board-col">
        <div className="card" style={{ padding: "var(--space-md)" }}>
          {/* Mode tabs */}
          <div
            style={{ display: "flex", gap: 6, marginBottom: "var(--space-md)" }}
          >
            {[
              {
                id: "game",
                label: "Game Line",
                active: !explorationMode && !guessMode,
              },
              { id: "explore", label: "⚡ Explore", active: explorationMode },
              {
                id: "guess",
                label: "🎯 Guess",
                active: guessMode && !explorationMode,
              },
            ].map((tab) => (
              <button
                key={tab.id}
          ...
          >
            ...
            >
              <AnalyzeEvalBar pct={evalBarPct} orientation={boardOrientation} />
            </div>
          </div>
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  {liveEval?.nps && !isEvaluating && (
                    ...
                    >
                      {Math.round(liveEval.nps / 1000)}k nps
                    </div>
                  )}
                  {liveEval?.depth && !isEvaluating && (
                    <div
                      style={{
                        fontSize: 10,
                        color: "var(--text-muted)",
              ...
              {!isEvaluating && liveEval?.lines?.length > 0 ? (
                      ...
                      style={{
                        ...
                        background:
                          i === 0
                            ...
                            : "var(--bg-subtle)",
                        border: `1px solid ${i === 0 ? "rgba(201,168,76,0.25)" : "var(--border-dim)"}`,
                ...
                >
                  {isEvaluating
                    ? "Evaluating…"
                    : "Navigate to a position to evaluate"}
                </div>
              )}
            </div>
          )}
            ...
            {!explorationMode && (
              <div className="board-controls">
                ...
                <button
                  ...
                  disabled={currentMoveIndex >= gameData.length - 1}
                >
                  ▶
                </button>
                <button
            ...
            {!explorationMode && improvementIndices.length > 0 && (
              <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                <button
                  ...
                  onClick={goToPrevImprovement}
                  disabled={
                    !improvementIndices.some((i) => i < currentMoveIndex)
                  }
                >
                  ◀ Prev improvement
                </button>
      ...
      <div className="analyze-study-col">
        ...
        >
          ...
          >
            ...
            <select
              value={evalDepth}
              onChange={(e) => setEvalDepth(e.target.value)}
              style={{
                background: "var(--bg-elevated)",
        ...
        {currentMove && !explorationMode && (
          <div className="card move-study-card">
            ...
            >
              Move {Math.ceil(currentMove.move_number / 2)}
              {currentMove.move_number % 2 !== 0 ? "." : "…"}
            </div>
              ...
              <div style={{ marginBottom: "var(--space-md)" }}>
                ...
                <div
                  style={{
                    ...
                    background: "var(--bg-elevated)",
                    borderRadius: "var(--radius-md)",
                    border: `2px solid ${getCplColor(currentMove.cpl)}33`,
                  }}
                >
                  <span
                    style={{
                  ...
                  <div>
                    <div
                      style={{
                        fontSize: 13,
                        fontWeight: 700,
                        color: getCplColor(currentMove.cpl),
                      }}
                    >
                      {getCplLabel(currentMove.cpl)}
                    </div>
                    {currentMove.cpl > 0 && (
                      <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                        −{Math.round(currentMove.cpl)} cp
                      </div>
                    )}
                  </div>
                </div>
              ...
              guessSan !== currentMove.san && (
                <div
                  style={{
                    ...
                    borderRadius: "var(--radius-md)",
                    border: "1px solid rgba(91,115,232,0.3)",
                  }}
                >
                  <div
                    style={{
            ...
            {(!guessMode || guessSubmitted) &&
              (() => {
                const bestMove =
                  liveEval?.lines?.[0]?.best_move || currentMove?.best_move;
                  ...
                  <div style={{ marginBottom: "var(--space-md)" }}>
                    ...
                    <div
                      style={{
                        ...
                        borderRadius: "var(--radius-md)",
                        border: "2px solid rgba(76,175,140,0.3)",
                      }}
                    >
                      <span
                        style={{
              ...
              <button
                ...
                onClick={() => {
                  const nextIdx = mistakesOnly
                    ? (improvementIndices.find((i) => i > currentMoveIndex) ??
                      Math.min(gameData.length - 1, currentMoveIndex + 1))
                    : Math.min(gameData.length - 1, currentMoveIndex + 1);
                  goToMove(nextIdx);
                }}
            ...
            {guessMode && (
              ...
              >
                ...
                >
                  Only show positions with mistakes
                </label>
              </div>
            )}
          </div>
          ...
          <div className="card move-study-card">
            ...
            <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>
              {explorationMoves.length === 0
                ? "Starting from current position. Drag pieces to explore."
                : `Line: ${explorationMoves.join(" ")}`}
            </div>
            {liveEval?.score && (
              <div
                style={{
