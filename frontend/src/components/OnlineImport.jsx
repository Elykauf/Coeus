...
function ResultBadge({ result }) {
  ...
  return (
    <span
      style={{ color: "#888888", fontWeight: 700, fontSize: 11, flexShrink: 0 }}
    >
      0.0
    </span>
  );
...
export default function OnlineImport() {
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        if (
          ...
          gamesRef.current.length < totalRef.current
        ) {
  ...
  async function fetchArchives() {
    ...
    try {
      ...
      setArchives(parsed);
      if (parsed.length === 0)
        setError("No game archives found for this username");
    } catch (e) {
      setError(
        e.response?.data?.detail ||
          "Failed to fetch archives — check the username",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function fetchLichessGames() {
    ...
    fetchPageRef.current = async (offset) => {
      ...
      try {
        ...
        setError(
          e.response?.data?.detail ||
            "Failed to fetch games — check the username",
        );
      } finally {
        setLoadingMore(false);
      }
  ...
  async function importGames() {
    ...
    if (toImport.length === 0) {
      toast("No games with PGN data selected", "warning");
      return;
    }
    ...
    setError(null);
    const endpoint =
      platform === "lichess" ? "/api/lichess/import" : "/api/chesscom/import";
    try {
      ...
      if (depth) {
        toast(
          `Queued ${imported} game${imported !== 1 ? "s" : ""} for analysis`,
          "success",
        );
      ...
      if (failed > 0)
        toast(
          `${failed} game${failed !== 1 ? "s" : ""} failed to import`,
          "error",
        );
    } catch (e) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter")
              platform === "chesscom" ? fetchArchives() : fetchLichessGames();
          }}
        />
        ...
        {platform === "lichess" && (
          <>
            ...
            <input
              ...
              max={300}
              onChange={(e) =>
                setLichessMax(Math.min(300, Math.max(1, +e.target.value)))
              }
            ...
            >
              {importResult.imported > 0 && (
                <span style={{ color: "#00C805", fontWeight: 600 }}>
                  {depth
                    ? `Queued ${importResult.imported}`
                    : `Imported ${importResult.imported}`}
                </span>
              )}
              {importResult.skipped > 0 && (
                <span> · {importResult.skipped} already existed</span>
              )}
