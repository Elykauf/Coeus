...
import { fmt, getAccuracy, getGrade, getRatingDelta } from "../utils/chess";
import MoveList from "./MoveList";
import { useToast } from "./Toast";
import { ConfirmModal, FairPlayPanel } from "./ui";
...
function Review({ analysis }) {
  ...
  const { currentFen, lastMoveSquares } = useMemo(() => {
    ...
    const squares = last
      ? {
          [last.from]: { backgroundColor: highlight },
          [last.to]: { backgroundColor: highlight },
        }
      : {};
    ...
    const handler = (e) => {
      ...
      if (e.key === "ArrowRight") {
        ...
        setCurrentMoveIndex((prev) =>
          Math.min(refinedData.length - 1, prev + 1),
        );
      }
      if (e.key === "Home") {
        e.preventDefault();
      ...
      if (e.key === "End") {
        e.preventDefault();
        setAutoPlay(false);
        setCurrentMoveIndex(refinedData.length - 1);
      }
    };
      ...
      <div className="review-grid">
        ...
        <div className="review-right-col">
          <div>
            <div
              className="skeleton-pulse"
              style={{ width: "60%", height: 28, marginBottom: 8 }}
            />
            <div
              className="skeleton-pulse"
          ...
          <div className="flex-gap">
            ...
            >
              <div>
                <div
                  className="skeleton-pulse"
                  style={{ width: 100, height: 11, marginBottom: 8 }}
                />
            ...
            >
              ...
              <div>
                <div
                  className="skeleton-pulse"
                  style={{ width: 80, height: 11, marginBottom: 8 }}
                />
                <div
                  className="skeleton-pulse"
  ...
  const confirmReanalyze = async () => {
    ...
    try {
      ...
      toast(
        `Reanalysis failed: ${err.response?.data?.detail || err.message}`,
        "error",
      );
    } finally {
  ...
  const avgAcc =
    relevantMoves.length > 0
      ? relevantMoves.reduce((sum, m) => sum + getAccuracy(m.cpl || 0), 0) /
  ...
  return (
    <>
      <div className="review-grid">
        ...
        <div className="review-left-col">
          ...
          <div className="stat-card--sm">
            ...
            <div
              className={`board-wrapper${boardPing ? " board-ping" : ""}`}
              style={{
                borderRadius: "4px",
                overflow: "hidden",
                border: "1px solid var(--border-dim)",
            ...
            {blunderCount > 0 && (
              <button
                className={`btn btn-ghost ${autoPlay ? "stop" : ""}`}
                style={{
                  width: "100%",
                  marginTop: "12px",
              ...
              >
                {autoPlay ? <SquareTerminal size={14} /> : <Play size={14} />}{" "}
        ...
        <div className="review-right-col">
            ...
            >
              <div>
                ...
                <div className="hero-stat">
                  <span className={avgAcc >= 90 ? "accuracy-glow" : ""}>
                    {avgAcc.toFixed(1)}
          ...
          <div className="stat-card">
            <div className="section-header">
              <h3 className="label-row">
                ...
                Move-by-Move Rating Delta
              </h3>
            </div>
            <p className="text-sm" style={{ margin: "4px 0 0 0" }}>
              Stockfish's best move converted to Elo points. Click any bar to
              view the position.
            </p>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart
                data={chartData.filter((d) => d.isRelevant)}
                ...
                <Tooltip
                  ...
                  labelFormatter={(l) => {
                    const entry = chartData.find(
                      (d) => d.move === l && d.isRelevant,
                    );
                    return entry
                      ? `Move ${l}: ${entry.userMove} vs ${entry.bestMove}`
                      : `Move ${l}`;
                  }}
                />
                <Bar
          ...
          <div className="stat-card">
            ...
            <p className="text-sm" style={{ margin: "4px 0 0 0" }}>
              Bar background indicates Opening, Middlegame, and Endgame.
            </p>
            <ResponsiveContainer width="100%" height={100}>
              <ScatterChart onClick={handleChartClick}>
                ...
                {/* Phase Backgrounds */}
                {chartData.find((d) => d.phase === "opening") && (
                  <ReferenceArea
                    x1={chartData.find((d) => d.phase === "opening").move}
                    x2={
                      chartData.filter((d) => d.phase === "opening").pop().move
                    }
                    fill="var(--phase-opening)"
                    fillOpacity={0.1}
                  />
                )}
                {chartData.find((d) => d.phase === "middlegame") && (
                  <ReferenceArea
                    x1={chartData.find((d) => d.phase === "middlegame").move}
                    x2={
                      chartData.filter((d) => d.phase === "middlegame").pop()
                        .move
                    }
                ...
                {chartData.find((d) => d.phase === "endgame") && (
                  <ReferenceArea
                    x1={chartData.find((d) => d.phase === "endgame").move}
                    x2={
                      chartData.filter((d) => d.phase === "endgame").pop().move
                    }
                ...
                <Scatter
                  name="Mistakes & Blunders"
                  data={chartData.filter((d) => d.isRelevant && d.cpl > 40)}
          ...
          {hasTime && (
            <div className="stat-card">
              ...
              <p className="text-sm" style={{ margin: "4px 0 0 0" }}>
                Green: The Zone | Red: Overthinking/Panic
              </p>
              <ResponsiveContainer width="100%" height={260}>
                <ScatterChart onClick={handleChartClick}>
                  ...
                  <Scatter
                    name="Moves"
                    data={chartData.filter((d) => d.isRelevant && d.time !== 0)}
                    fill="var(--text-primary)"
                  >
                    {chartData
                      .filter((d) => d.isRelevant && d.time !== 0)
                      .map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
      ...
      {showReanalyzeConfirm && (
        <ConfirmModal
          title="Reanalyze Game"
          message="This will clear the current analysis and re-run Stockfish with your configured options. Continue?"
