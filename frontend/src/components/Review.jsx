...
import { Target, Clock, AlertCircle, Zap, Play, SquareTerminal } from 'lucide-react'
import { useBoardColors } from '../hooks/useBoardColors'
import MoveList from './MoveList'
import TimeInsightsPanel from './ui/TimeInsightsPanel'
import { fmt, getAccuracy, getGrade, getCplColor, getCplBadge, evalToPercent } from '../utils/chess'
  ...
  return (
    <div className="review-grid">
        ...
        <div className="review-right-col">
           ...
           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
             <div>
               ...
               <div style={{ color: 'var(--text-secondary)', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <span>{meta.eco ? `${meta.eco} ` : ''}{meta.opening ? `· ${meta.opening} ` : ''}{meta.timeControl ? `· ⏱ ${meta.timeControl} ` : ''}· Stockfish 16.1</span>
                  {analysis?.depth && (
                    <span style={{
           ...
           {/* Accuracy Trend (Area Chart) — FinTech Style */}
           <div className="stat-card">
              <div className="section-header">
                <h3 className="label-row"><Zap size={16} style={{ color: 'var(--accent)'}} /> Performance</h3>
              </div>
              <p className="text-sm" style={{ margin: '4px 0 0 0' }}>Click any point to view the position on the board.</p>
              <ResponsiveContainer width="100%" height={220}>
                 <AreaChart data={chartData.filter(d => d.isRelevant)} onClick={handleChartClick}>
                    <defs>
                    ...
                    <YAxis domain={[0, 100]} stroke="#555" tick={{ fontSize: 10 }} tickLine={false} axisLine={false} width={30} />
                    <Tooltip cursor={{ stroke: 'rgba(34,197,94,0.2)' }} contentStyle={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)' }} itemStyle={{ color: 'var(--accent)' }} formatter={fmt} />
                    <ReferenceLine y={90} stroke="var(--accent)" strokeDasharray="3 3" strokeOpacity={0.3} />
                    <Area type="linear" dataKey="accuracy" stroke="var(--accent)" strokeWidth={2} fillOpacity={1} fill="url(#accGradient)" activeDot={{ r: 6, fill: 'var(--accent)', stroke: '#050505', strokeWidth: 2 }} />
                 </AreaChart>
              </ResponsiveContainer>

              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: -12 }}>
                <div className="legend-item">
                  <div style={{ width: 10, borderBottom: '1px dashed var(--accent)' }} /> Master Level Target (90%)
                </div>
              </div>
           </div>
           ...
           <div className="stat-card">
              ...
              <ResponsiveContainer width="100%" height={100}>
                <ScatterChart onClick={handleChartClick}>
                  ...
                  <Tooltip cursor={{ strokeDasharray: '3 3', stroke: 'rgba(255,255,255,0.2)' }} contentStyle={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)' }} formatter={fmt} />
             ...
             <div className="stat-card">
                ...
                <ResponsiveContainer width="100%" height={260}>
                   <ScatterChart onClick={handleChartClick}>
                      ...
                      <Tooltip cursor={{ strokeDasharray: '3 3' }} contentStyle={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)' }} formatter={fmt} />
